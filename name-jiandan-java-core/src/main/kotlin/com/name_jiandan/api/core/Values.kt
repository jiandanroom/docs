package com.name_jiandan.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { NameJiandanInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw NameJiandanInvalidDataException("`$name` is not set")
            is JsonNull -> throw NameJiandanInvalidDataException("`$name` is null")
            else -> throw NameJiandanInvalidDataException("`$name` is invalid, received $this")
        }
